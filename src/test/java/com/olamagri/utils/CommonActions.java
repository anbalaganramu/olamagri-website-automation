...
public class CommonActions {
	...
	public void waitForPageLoad(WebDriver driver, long timeOutInSeconds) {
	    new WebDriverWait(DriverClass.driver, Duration.ofSeconds(timeOutInSeconds)).until((ExpectedCondition<Boolean>) wd ->
	...
	public void validateURLInNewTab(String expectedURL) {
		String originalHandle = driver.getWindowHandle();
		for (String handle : driver.getWindowHandles()) {
			if (!handle.equals(originalHandle)) {
				driver.switchTo().window(handle);
	...
	public void validatePageTitleInNewTab(String expectedPageTitle) {
		String originalHandle = driver.getWindowHandle();
		for (String handle : driver.getWindowHandles()) {
			if (!handle.equals(originalHandle)) {
				driver.switchTo().window(handle);
				...
				try {
					...
					// TODO Auto-generated catch block
					//e.printStackTrace();
					System.out.println("There is an error in the page: "+expectedPageTitle);
				}
				driver.close();
			}
		}
	...
	public void scrollVerticalTo(String y) {
		((JavascriptExecutor) driver).executeScript("window.scrollTo(0,"+y+")");
	}
